...
def broadcast_sender_nodes_to_edges(
  ...
  with tf.name_scope(name):
    return tf.gather(graph.nodes, graph.senders[:,0])
...
def broadcast_receiver_nodes_to_edges(
  ...
  with tf.name_scope(name):
    return tf.gather(graph.nodes, graph.receivers[:,0])
...
class EdgeBlock(_base.AbstractModule):
  ...
  def _build(self, graphs):
    ...
    if self._use_receiver_nodes:
      for graph in graphs:
        r_to_e = broadcast_receiver_nodes_to_edges(graph)
        edges_to_collect.append(r_to_e)

    if self._use_sender_nodes:
      for graph in graphs:
        s_to_e = broadcast_sender_nodes_to_edges(graph)
        edges_to_collect.append(s_to_e)

    if self._use_globals:
      for graph in graphs:
        g_to_e = broadcast_globals_to_edges(graph)
        edges_to_collect.append(g_to_e)

    collected_edges = tf.concat(edges_to_collect, axis=-1)
    updated_edges = self._edge_model(collected_edges)
    return [graph.replace(edges=updated_edges) for graph in graphs]
